...
import { getBookingAvailabilityRecords } from "@/lib/supabase/booking-data";
...
export default async function BookingPage({ searchParams }: BookingPageProps) {
  ...
  const [{ activeVenue: venue, venues }, availability, plannerLibrary] = await Promise.all([
    getVenuePageData(params.venue),
    getBookingAvailabilityRecords(),
    getPlannerLibraryData(),
  ]);
  const bookingVideoSrc = venue.bookingVideoSrc;
      ...
      <div className="absolute inset-0 left-1/2 -z-10 w-screen -translate-x-1/2 overflow-hidden">
        {bookingVideoSrc ? (
          <video
            ...
            playsInline
          >
            <source src={bookingVideoSrc} type="video/mp4" />
          </video>
        ) : null}
